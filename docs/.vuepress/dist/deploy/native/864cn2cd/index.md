---
url: /deploy/native/864cn2cd/index.md
---
## 简介
...
* 下载配套安装包，并上传到 Linux 系统中, 标准 Linux 系统（X86 / X86\_64）请执行如下命令安装：
...
2. `/etc/systemd/resolved.conf`{.fp} 取消注释并修改以下行
...
::: info `/etc/smartdns/smartdns.conf`{.fp} 配置包含如下基本内容：
